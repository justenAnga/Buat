...
class menu_create:
    def __init__(self):
        ...
        update    = input('%s[%s•%s] %sAuto Update Info Akun [%sy%s/%st%s] : '%(M,P,M,P,M,P,H,P)).lower()
...
class create_fb:
    ...
    def __init__(self):
        ...
        else : self.ua = 'Mozilla/5.0 (Linux; Android 11; vivo 1918 Build/RP1A.200720.012; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/112.0.0000.00 Mobile Safari/537.36'
    ...
    def get_code_cryptogmail(self):
        url = f'https://cryptogmail.com/api/emails?inbox={self.email}'
    ...
    def scrap5(self,req): #--> Confirm Code
        ...
        try:
            ...
            self.printing('CP')
    def scrap5gagal(self): #--> Confirm Code
        print('\rLolos Tahap 4                    ',end='');sys.stdout.flush()
        head_khusus = self.headers_get.update({'user-agent':self.ua_wind})
        cok  = '; '.join([str(x)+"="+str(y) for x,y in self.xyz.cookies.get_dict().items()])
        cok += self.perangkat
        try:
            if   web_email in ['c','cryptogmail','1','01','a']: code = self.get_code_cryptogmail()
            ...
            req = bs(self.xyz.get('https://www.facebook.com/confirmemail.php?',headers=head_khusus,cookies={'cookie':cok},allow_redirects=True).content,'html.parser')
            fom = req.find('form',{'method':'post'})
            data = {
                'fb_dtsg'         : re.search('name="fb_dtsg" type="hidden" value="(.*?)"',str(fom)).group(1),
                'jazoest'         : re.search('name="jazoest" type="hidden" value="(.*?)"',str(fom)).group(1),
            ...
            cok  = '; '.join([str(x)+"="+str(y) for x,y in self.xyz.cookies.get_dict().items()])
            cok += self.perangkat
            next = 'https://www.facebook.com' + fom['action']
            pos = bs(self.xyz.post(next,data=data,headers=head_khusus,cookies={'cookie':cok},allow_redirects=True).content,'html.parser')
            self.semi_final()
        except Exception as e:
            self.printing('CP')
    ...
    def semi_final(self): #--> Sortir
        ...
        try:
            id = re.search('c_user=(.*?);',cok).group(1)
            req = bs(self.xyz.get(f'https://m.facebook.com/profile.php?id={id}',headers=self.headers_get,cookies={'cookie':cok},allow_redirects=True).content,'html.parser')
            #if 'Telkomsel' in str(req): self.zero_optin()
            stat = req.find('title').text
            if 'konfirmasi' in str(stat).lower(): self.printing('CP')
            else:
                if update in ['1','01','y','a']:
                    ...
                    self.account_follow = ['1827084332','100000415317575']
                    city = 'Yogyakarta'
    ...
    #--> Auto Update Akun
    def cari_teman(self): #--> Skip Cari Terman
        try:
            req = bs(self.xyz.get('https://m.facebook.com/',cookies={'cookie':self.cookie},headers=self.headers_get,allow_redirects=True).content,'html.parser')
            url = ['https://m.facebook.com'+x['href'] for x in req.find_all('a',href=True) if 'nux/wizard' in str(x['href'])][0]
            roq = self.xyz.get(url,cookies={'cookie':self.cookie},headers=self.headers_get,allow_redirects=True)
            print('\rBerhasil Skip Teman                    ',end='');sys.stdout.flush()
        except Exception as e: pass
    def profil(self): #--> Edit Foto Profil
        try:
            fot = urllib.request.urlopen(self.foto)
            url = 'https://mbasic.facebook.com/profile_picture/'
            req = bs(self.xyz.get(url,cookies={'cookie':self.cookie},headers=self.head1).content,'html.parser')
            ...
            dat = {
                ...
                'submit'  : 'Simpan'}
            fil = {'pic' : fot}
            pos = bs(self.xyz.post(raq['action'],data=dat,files=fil,cookies={'cookie':self.cookie},headers=self.head1).content,'html.parser')
    ...
    def cover(self): #--> Edit Foto Sampul
        try:
            ...
            dat = {
                ...
                'submit'                   : 'Unggah'}
            fil = {'file1' : fot}
            pos = bs(self.xyz.post('https://mbasic.facebook.com'+raq['action'],data=dat,files=fil,cookies={'cookie':self.cookie},headers=self.head1).content,'html.parser')
    ...
    def bio(self): #--> Edit Bio
        try:
            ...
            dat = {
                ...
                'submit'          : 'Simpan'}
            pos = bs(self.xyz.post('https://mbasic.facebook.com'+raq['action'],data=dat,cookies={'cookie':self.cookie},headers=self.head1).content,'html.parser')
    ...
    def auto_follow(self): #--> Auto Follow
        ...
        for x in self.account_follow:
            ...
            try:
                ...
                get = self.xyz.get('https://mbasic.facebook.com'+raq['href'],cookies={'cookie':self.cookie},headers=self.head1)
                l += 1
                print('\rBerhasil Follow Akun %s                    '%(str(l)),end='');sys.stdout.flush()
            except Exception as e: pass
    ...
    def logout(self): #--> Logout Manual
        try:
            ...
            req = bs(self.xyz.get('https://mbasic.facebook.com/me',cookies={'cookie':cok},headers=self.headers_get,allow_redirects=True).content,'html.parser')
            url = ['https://mbasic.facebook.com'+x['href'] for x in req.find_all('a',href=True) if 'logout.php' in str(x['href'])][0]
            cok  = '; '.join([str(x)+"="+str(y) for x,y in self.xyz.cookies.get_dict().items()]) + self.perangkat
            roq = self.xyz.get(url,cookies={'cookie':cok},headers=self.headers_get,allow_redirects=True)
